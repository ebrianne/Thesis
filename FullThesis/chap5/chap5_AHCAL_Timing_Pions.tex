...
In this chapter, the recorded pion data is compared to \geant v10.1 simulations using different hadronic models as explained in chapter \ref{chap:G4Simulation}. Just as a reminder, the hit time of a channel is referred to as \textit{the time of the first hit} because only the first hit per channel is registered during a bunch-crossing (multiple hits could occur in a channel but only the first one is kept by the SPIROC2b).
